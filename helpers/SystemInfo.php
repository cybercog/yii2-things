...
/**
 * Class SystemInfo  - спёрто с github.com/trntv/system-info
 */
class SystemInfo
{
    /**
...
// todo: Function
    ...
    {
        $dirsize = \Yii::$app->cache->get('size_' . $dir);
        if (!is_dir($dir)) {
            return 0;
        }
        if (!$dirsize) {
            $dirsize = 0;
            $files = FileHelper::findFiles($dir, ['except' => ['*.html', '*.php', '.htaccess']]);
            foreach ($files as $file) {
                $dirsize += filesize($file);
            }
            \Yii::$app->cache->set('size_' . $dir, $dirsize, 30);
        }
        return $dirsize;
    }
